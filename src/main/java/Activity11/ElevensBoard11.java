...
public class ElevensBoard11 extends Board11 {
	...
	private List<Integer> findPairSum11(List<Integer> selectedCards) {
		...
		List<Integer>eleven = new ArrayList<>();
	...
	private List<Integer> findJQK(List<Integer> selectedCards) {
		...
		for (int val : selectedCards) {
			String rank = cardAt(val).rank();
			if (rank.equals("jack")) jack = val;
			else if (rank.equals("queen")) queen = val;
			else if (rank.equals("king")) king = val;
		}
	...
	private boolean playJQKIfPossible() {
		/* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
		List<Integer> indexes = cardIndexes();
		List<Integer> jqk = findJQK(indexes);
		if (jqk.size() == 3) {
